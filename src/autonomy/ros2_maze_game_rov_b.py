    ...
    def process_message(self):
        ...
        try:
            result = subprocess.run(
                ["ollama", "run", "ros2_robot_game_rov_b"],
                input=prompt,
                text=True,
                capture_output=True,
                check=True
            ...
            self.counter += 1
            self.get_logger().info(f"LLM Agent B (call #{self.counter}): {llm_output}")
        ...
        for line in lines:
            if "robot_a:" in line:
                ...
                if len(parts) > 1:
                    ...
                    self.get_logger().info(f"Published to /actions_from_llm_agent_rov_b: {action_str}")
